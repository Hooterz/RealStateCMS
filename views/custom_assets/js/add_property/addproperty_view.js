document.addEventListener("DOMContentLoaded", () => {
    ...
    // Comprobaciones antes de mandar el formulario
    document.querySelector('.intro-single > div:nth-child(1) > form:nth-child(1)').addEventListener('submit', (e) => {
        let tipo_de_propiedad = document.querySelector('#type_select');
        let localizacion = document.querySelector('#location');
        if(tipo_de_propiedad.value === 'default' || localizacion.value === 'default'){
            alert("No has rellenado el 'Tipo de adición' o la 'Ciudad'");
            e.preventDefault();
        } 
    });
});
