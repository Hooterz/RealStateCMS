...
use RealStateModel\Map\FeatureTableMap;
...
abstract class Feature implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\RealStateModel\\Map\\FeatureTableMap';
    ...
    /**
     * The value for the feature_content field.
     *
     * @var        string|null
     */
    ...
    /**
     * Initializes internal state of RealStateModel\Base\Feature object.
     */
    ...
    /**
     * Get the [feature_content] column value.
     *
     * @return string|null
     */
     ...
     * @return $this|\RealStateModel\Feature The current object (for fluent API support)
     */
    public function setFeatureId($v)
    {
        ...
        if ($this->feature_id !== $v) {
            $this->feature_id = $v;
            $this->modifiedColumns[FeatureTableMap::COL_FEATURE_ID] = true;
        }
    ...
    /**
     * Set the value of [feature_content] column.
     *
     * @param string|null $v New value
     * @return $this|\RealStateModel\Feature The current object (for fluent API support)
     */
    public function setFeatureContent($v)
    {
        if ($v !== null) {
            $v = (string) $v;
        }

        if ($this->feature_content !== $v) {
            $this->feature_content = $v;
            $this->modifiedColumns[FeatureTableMap::COL_FEATURE_CONTENT] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : FeatureTableMap::translateFieldName('FeatureId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->feature_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : FeatureTableMap::translateFieldName('FeatureContent', TableMap::TYPE_PHPNAME, $indexType)];
            $this->feature_content = (null !== $col) ? (string) $col : null;
            $this->resetModified();
            ...
            return $startcol + 2; // 2 = FeatureTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\RealStateModel\\Feature'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(FeatureTableMap::COL_FEATURE_ID)) {
            $modifiedColumns[':p' . $index++]  = 'feature_id';
        }
        if ($this->isColumnModified(FeatureTableMap::COL_FEATURE_CONTENT)) {
            $modifiedColumns[':p' . $index++]  = 'feature_content';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'feature_content':
                        $stmt->bindValue($identifier, $this->feature_content, PDO::PARAM_STR);
                        break;
                }
            }
            $stmt->execute();
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['Feature'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Feature'][$this->hashCode()] = true;
        $keys = FeatureTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getFeatureId(),
            $keys[1] => $this->getFeatureContent(),
        );
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\RealStateModel\Feature
     */
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[1], $arr)) {
            $this->setFeatureContent($arr[$keys[1]]);
        }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\RealStateModel\Feature The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(FeatureTableMap::COL_FEATURE_ID)) {
            $criteria->add(FeatureTableMap::COL_FEATURE_ID, $this->feature_id);
        }
        if ($this->isColumnModified(FeatureTableMap::COL_FEATURE_CONTENT)) {
            $criteria->add(FeatureTableMap::COL_FEATURE_CONTENT, $this->feature_content);
        }
    ...
    {
        $criteria = ChildFeatureQuery::create();
        $criteria->add(FeatureTableMap::COL_FEATURE_ID, $this->feature_id);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \RealStateModel\Feature (or compatible) type.
    ...
    {
        $copyObj->setFeatureContent($this->getFeatureContent());
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \RealStateModel\Feature Clone of current object.
     * @throws PropelException
     */
     ...
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildFeature The current object (for fluent API support)
     */
    public function setPropertyFeatures(Collection $propertyFeatures, ConnectionInterface $con = null)
    {
        ...
        foreach ($propertyFeaturesToDelete as $propertyFeatureRemoved) {
            $propertyFeatureRemoved->setFeature(null);
        }
    ...
    protected function doAddPropertyFeature(ChildPropertyFeature $propertyFeature)
    {
        ...
        $propertyFeature->setFeature($this);
    }
    ...
    public function removePropertyFeature(ChildPropertyFeature $propertyFeature)
    {
        if ($this->getPropertyFeatures()->contains($propertyFeature)) {
            ...
            $propertyFeature->setFeature(null);
        }
     ...
     * @return ObjectCollection|ChildPropertyFeature[] List of ChildPropertyFeature objects
     */
    public function getPropertyFeaturesJoinProperty(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildPropertyFeatureQuery::create(null, $criteria);
